# M1.7 human recording and physical swatter
...
State fractions include dead ticks. Escape latency is the first new ESCAPE after
...
The mouse is a desired hand target, never a teleport. Recent pointer history is
...
| Position gain / velocity time constant | 12/s / 0.045 s |
...
The active ROOM record is `results/game/calibration_room_m1_7_edge.json` (threshold
1.70), with exact config/runtime provenance. Run:
...
The inertial paddle settles for 100 ticks (2 seconds) before measurement.
...
created unreachable edge and corner regions. The active config version 8 sets

...
def escape_decoder_spec(config: dict) -> dict | None:
    ...
    for key in ("sustained_threshold", "fast_threshold"):
        value = spec[key]
        if isinstance(value, bool) or not isinstance(value, (int, float)) \
                or not math.isfinite(value) or value <= 0.0:
            raise ValueError(f"policy.escape_decoder.{key} must be a positive number")
        spec[key] = float(value)
    if spec["fast_threshold"] <= spec["sustained_threshold"]:
        raise ValueError("policy.escape_decoder.fast_threshold must exceed sustained_threshold")
    return spec
...
def resolve_escape_threshold(config: dict, root: Path = ROOT) -> ThresholdSource:
    ...
    if decoder is not None:
        command = ("python tools/n2b_decoder_record.py" if decoder["kind"] == WINDOW_DECODER
                   else "python tools/n2_decoder_record.py")
